package sample.MEncrypt
{
   ...
   public function newThread(param1:int, param2:int, param3:int, param4:Vector.<int>) : *
   {
      ...
      _loc5_.setSharedProperty("flascc.threadArbMutex",sample.MEncrypt.threadArbMutex);
      var _loc6_:int = 0;
      while(_loc6_ < 32)
      {
         _loc5_.setSharedProperty("flascc.threadArbCond." + _loc6_,sample.MEncrypt.threadArbConds[_loc6_]);
         _loc6_++;
      }
      sample.MEncrypt.ram_init.shareable = true;
      _loc5_.setSharedProperty("flascc.ram",sample.MEncrypt.ram_init);
      ...
      for each(_loc8_ in sample.MEncrypt.workerInits)
      {
         _loc8_(_loc5_);
      }
      return _loc5_;
