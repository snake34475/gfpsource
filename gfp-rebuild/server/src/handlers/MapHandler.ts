...
export class MapHandler {
  ...
  private sendToFunc: (client: Client, commandId: number, data: any) => void;

  constructor(
    players: Map<number, Player>,
    broadcastFunc: (player: Player, data: any) => void,
    sendToFunc: (client: Client, commandId: number, data: any) => void
  ) {
  ...
  handleMapSwitch(client: Client, data: any): void {
    ...
    // 通知其他玩家该玩家离开了旧地图
    this.broadcastFunc({ id: -1, mapId: oldMapId, pos: { x: 0, y: 0 } } as Player, { userId: client.player.id });
  }

  private getMapConfig(mapId: number): any {
  ...
  private getSpawnPoint(mapId: number, x?: number, y?: number): { x: number, y: number } {
    const defaultPoints: Record<number, { x: number, y: number }> = {
      1001: { x: 500, y: 800 },
      1002: { x: 600, y: 900 },
      1003: { x: 700, y: 1000 },
      1004: { x: 400, y: 600 },
