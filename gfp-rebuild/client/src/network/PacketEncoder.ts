...
export class PacketEncoder {
  ...
  private static encodeBody(commandId: number, data: any[]): Uint8Array {
    const chunks: Uint8Array[] = [];
  ...
  private static encodeValue(value: any): Uint8Array {
    if (typeof value === "number") {
      ...
      const chunks: Uint8Array[] = [new Uint8Array(lengthView.buffer)];
  ...
  static encodeMove(
    ...
    return this.encode(CommandID.MOVE, [mapId, x, y, speed, moveType]);
  }

  static encodeStand(mapId: number, x: number, y: number, direction: number): ArrayBuffer {
    return this.encode(CommandID.STAND, [mapId, x, y, direction]);
  }

  static encodeJump(x: number, y: number): ArrayBuffer {
    return this.encode(CommandID.JUMP, [x, y]);
  }

  static encodeSkill(skillId: number, skillLv: number, x: number, y: number, targetId: number): ArrayBuffer {
    return this.encode(CommandID.ACTION_SKILL, [skillId, skillLv, x, y, targetId]);
  }
}