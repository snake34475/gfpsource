package com.gfp.app.task.tc
{
   ...
   public class TaskXML_TollgateEnterStatus extends TaskXML_Base
   {
      
      private var _tollgateId:uint;
      
      private var _enterStaus:uint;
      
      private var _tollgateCount:uint;
      
      public function TaskXML_TollgateEnterStatus()
      {
         super();
      }
      ...
      override public function setup(param1:uint, param2:uint, param3:String, param4:Boolean) : void
      {
         ...
         this._enterStaus = uint(_loc6_[1]);
         this._tollgateCount = uint(_loc6_[2]);
      }
      
      override protected function addListener() : void
      {
         TasksManager.addActionListener(TaskActionEvent.TASK_TOLLGATE_ENTER_STATUS,this._tollgateId + StringConstants.SIGN + this._enterStaus,this.onTollgateWin);
      }
      
      override public function uninit() : void
      {
         TasksManager.removeActionListener(TaskActionEvent.TASK_TOLLGATE_ENTER_STATUS,this._tollgateId + StringConstants.SIGN + this._enterStaus,this.onTollgateWin);
      }
         ...
         if(_loc2_)
         {
            ...
            if(_loc3_ < this._tollgateCount)
            {
               ...
               if(_loc3_ < this._tollgateCount)
               {
                  ...
                  TextAlert.show(this.getTaskProcName(_taskID,_proID) + " " + TextFormatUtil.getRedText(_loc3_.toString() + "/" + this._tollgateCount.toString()));
               }
               else
               {
                  ...
                  this.uninit();
                  TextAlert.show(this.getTaskProcName(_taskID,_proID) + " " + TextFormatUtil.getRedText(_loc3_.toString() + "/" + this._tollgateCount.toString()) + "(完成)");
               }
            }
            else
            {
