package com.gfp.app.mapProcess
{
   ...
   public class MapProcess_58 extends MapProcessAnimat
   {
      ...
      private var _tuduiPosArr:Array = [[1214,344],[832,592],[616,426]];
      ...
      private var _tuduiFlipIDArr:Array = [2251,2252,2253];
      
      private var _updateTimeID:int = 2261;
      
      private var _updateTuduiFlipID:int = 2254;
      
      private var _checkZujiCountID:int = 2231;
      
      private var _npc10155:SightModel;
      
      private var _whiteNpc:SightModel;
      
      private var _blackNpc:SightModel;
      
      private var _currentNpc:SightModel;
      ...
      {
         this.initNpc10155();
         this.initHallowmasNpc();
         this.initTaskEvent();
         this.initAnimatEvent();
         if(TasksManager.isReady(2029))
         {
            AnimatPlay.startAnimat("task2029_",1,true,12,0,false,true);
         }
         var _loc1_:SightModel = SightManager.getSightModel(10522);
         _loc1_.visible = false;
         if(Boolean(TasksManager.isCompleted(2032)) && !TasksManager.isCompleted(2033))
         {
            _loc1_.visible = true;
         }
         if(TasksManager.isProcess(2033,1))
         {
            TasksManager.taskProComplete(2033,1);
         }
      }
      ...
      private function initGreenDargonData() : void
      {
         ServerUniqDataManager.requestDateByType(ServerUniqDataManager.TYPE_DARGON_EGG);
         ServerUniqDataManager.addListener(ServerUniqDataEvent.EVENT_DARGON_EGG,this.onServerUniqData);
      }
      
      private function onServerUniqData(param1:ServerUniqDataEvent) : void
      {
         var _loc5_:SwapActionLimitInfo = null;
         ServerUniqDataManager.removeListener(ServerUniqDataEvent.EVENT_DARGON_EGG,this.onServerUniqData);
         ...
         while(_loc6_ < _loc3_.length)
         {
            ...
            if(_loc5_.dailyID == 100000010 && _loc5_.limitCount > 0)
            {
               this._npc10155.show();
            }
            _loc6_++;
      ...
      private function initNpc10155() : void
      {
         this._npc10155 = SightManager.getSightModel(10155);
         this.initGreenDargonData();
      }
      
      private function initHallowmasNpc() : void
      {
         this._blackNpc = SightManager.getSightModel(10103);
         this._whiteNpc = SightManager.getSightModel(10104);
         this.showHallowmasNpc();
      }
      
      private function showHallowmasNpc() : void
      {
         ...
         {
            this._blackNpc.hide();
            this._currentNpc = this._whiteNpc;
         }
         else
         {
            this._whiteNpc.hide();
            this._currentNpc = this._blackNpc;
         }
      }
      ...
      {
         if(param1.taskID == 2033 && param1.proID == 0)
         {
            PveEntry.instance.enterTollgate(595);
         }
      }
      ...
      private function onTaskAccept(param1:TaskEvent) : void
      {
         ...
         {
            case 1917:
               TasksManager.taskComplete(1917);
               break;
            case 2030:
               AnimatPlay.startAnimat("task2030_",0,true,0.1,0.1,false,true,false);
         }
      }
      
      private function initAnimatEvent() : void
      {
         AnimatPlay.addAnimatListener(AnimatEvent.ANIMAT_END,this.onAnimatEnd);
      }
      
      private function removeAnimatEvent() : void
      {
      ...
      private function onAnimatEnd(param1:AnimatEvent) : void
      {
         ...
         {
            case "task2029_1":
               TasksManager.taskComplete(2029);
               break;
            case "task2030_0":
               NpcDialogController.showForNpc(10155);
         }
      }
