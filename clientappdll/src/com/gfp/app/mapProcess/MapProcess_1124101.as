package com.gfp.app.mapProcess
{
   ...
   public class MapProcess_1124101 extends BaseMapProcess
   {
      ...
      private var _timeLimit:int = 120000;
      ...
      private var _maxBorn:int;
      
      private var _getExpSwapId:int = 8959;
      
      private var _getBornSwapId:int = 8960;
      
      public function MapProcess_1124101()
      {
         super();
      }
      ...
      {
         this._maxExp = Math.max(0,500 * 10000 - ActivityExchangeTimesManager.getTimes(this._getExpSwapId));
         this._maxBorn = Math.max(0,100 - ActivityExchangeTimesManager.getTimes(this._getBornSwapId));
         ...
         this._expBornStateBar.barMc.gotoAndStop(1);
      ...
      {
         SocketConnection.send(CommandID.FIGHT_ACTIVITY_EFFECT,9,0,0);
         this._skillMc.visible = false;
         this._skillBtn.filters = FilterUtil.GRAY_FILTER;
      ...
      private function onUserDied(param1:UserEvent) : void
      {
         ...
         if(_loc2_)
         {
            if(_loc2_.info.roleType == 12421)
            {
               this._exp += 2000;
            }
            else if(_loc2_.info.roleType == 12422)
            {
               this._born += 2;
            }
            else if(_loc2_.info.roleType == 14669)
            {
               this._exp += 10 * 10000;
               this._born += 5;
            }
            else if(_loc2_.info.roleType == 14670)
            {
               this._exp += 20 * 10000;
               this._born += 8;
            }
            this._power += 4;
            this._exp = Math.min(this._maxExp,this._exp);
            this._born = Math.min(this._maxBorn,this._born);
            if(this._expBornStateBar)
            {
               this._expBornStateBar.barMc.gotoAndStop(this._power);
            }
            ...
            if(this._exp < 10000)
            {
               this._expQianNum.value = this._exp;
               this._expQianNum.max_live = 4;
               this._cntMc["expNum"].visible = false;
               this._cntMc["expQianNum"].visible = true;
            }
            else
            {
               ...
               this._cntMc["expQianNum"].visible = false;
               this._expNum.max_live = 3;
               this._expNum.value = Math.floor(this._exp / 10000);
            }
            if(!this._bornNum)
            {
               this._bornNum = new NumSprite(this._cntMc["bornNum"],0);
            ...
            this._bornNum.value = this._born;
         }
      }
